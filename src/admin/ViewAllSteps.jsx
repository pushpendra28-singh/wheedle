...
const ViewAllSteps = ({ setActivePage }) => {
  ...
  const fetchSteps = async () => {
    ...
    try {
      ...
      setToast({ message: "Failed to fetch steps", type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${selectedIds.length} selected steps?`)) return;
    ...
    try {
      ...
      showError("Failed to delete some steps");
      fetchSteps();
    }
  };
  ...
  return (
    <div className="bg-white border border-gray-300 p-8 rounded-xl shadow-sm w-full relative">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">All Steps</h1>
          <p className="text-gray-500 text-sm mt-1">Total: {steps.length} steps</p>
        </div>
      ...
      {loading ? (
        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {currentItems.map((step) => (
                  <tr
                    key={step._id}
                    className={`border-b text-gray-800 text-base hover:bg-gray-50 transition ${selectedIds.includes(step._id) ? "bg-blue-50" : ""
                      }`}
                  >
                    <td className="p-4">
                      <input
                        type="checkbox"
                        className="w-5 h-5 cursor-pointer"
                        checked={selectedIds.includes(step._id)}
                        onChange={() => toggleSelect(step._id)}
                      />
                    </td>
                    ...
                    <td className="p-4">
                      {step.image ? (
                        <img
                          src={`${API_BASE_URL}/uploads/${step.image}`}
                    ...
                    <td className="p-4 max-w-xs truncate">{step.description}</td>
                    <td className="p-4 text-gray-500">
                      {new Date(step.createdAt).toLocaleDateString()}
                    </td>
                    <td className="p-4 text-center">
                      <Trash2
                        className="text-red-500 hover:text-red-700 transition transform hover:scale-110 cursor-pointer inline-block"
            ...
            {steps.length === 0 && (
              <div className="text-center py-10 text-gray-500">No steps found.</div>
            )}
          </div>
