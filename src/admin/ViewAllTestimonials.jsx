...
const ViewAllTestimonials = ({ onClose }) => {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${selectedIds.length} selected testimonials?`)) return;
    ...
    try {
      ...
      showError("Failed to delete some testimonials");
      fetchTestimonials();
    }
  };
  ...
  return (
    <div className="bg-white border border-gray-300 p-8 rounded-xl shadow-sm w-full relative">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-3xl font-semibold text-gray-800">All Testimonials</h2>
          <p className="text-gray-500 text-sm mt-1">Total: {testimonials.length} testimonials</p>
        </div>
        ...
        <>
          <div className="overflow-x-auto">
            ...
            {testimonials.length === 0 && (
              <div className="text-center py-10 text-gray-500">No testimonials found.</div>
            )}
          </div>
